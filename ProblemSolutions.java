...
public class ProblemSolutions {
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        ...
        Arrays.sort(asteroids); // Sort asteroids in ascending order of mass
        // Loop through all the asteroids
        for (int i = 0; i < asteroids.length; i++) {
            // If the planet is bigger and can destroy this asteroid
            if (mass >= asteroids[i]) {
                mass += asteroids[i];// Increase its mass
            } else {
                return false; // Planet is too small to destroy the asteroid
            }
        }
        return true; // All asteroids destroyed successfully
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        Arrays.sort(people); // Sort people array based on their weight
        int sleds = 0; // Sleds used count initilized to zero
        int light = 0; // pointer to the lightest person
        int heavy = people.length - 1; // pointer to the heaviest person

        while (light <= heavy) { // loop while there are people/two pointers meet
            if (people[light] + people[heavy] <= limit) { // If they fit in one sled, count them together
                light++; // Move past by the light person
                heavy--; // Move past by the heavy person
            } else {
                heavy--; // Otherwise, count in the heavy person alone
            }
            sleds++; // Make one sled used
        }
        return sleds; // Return total number of sleds needed
